...
const pageTitle = "Couchbase Chatbot";
...
function chainWebpackWorker(config, destDir = "", srcDir = "src/lib") {
  ...
  const workerFilename = buildType.isProd
    ? `${workerBaseFilename}.min.js`
    : `${workerBaseFilename}.js`;

  // configure worker loader
  config.module
    .rule("worker")
    .test(/-worker\.js$/)
    .include.add(path.resolve(__dirname, srcDir))
    .end()
    .use("worker-loader")
...
function chainWebpackApp(config, destDir = "") {
  config.output.filename(buildType.isProd ? "[name].min.js" : "[name].js");
  ...
  config
    ...
    .tap((args) => {
      ...
      args[0].patterns.unshift(
        // favicon.png
        {
          from: getAssetPath(favIconPath, defaultPath),
          to: `${distDir}/favicon.png`,
        },
        // logo.png
        {
          from: getAssetPath(logoPath, defaultPath),
          to: `${distDir}/logo.png`,
        }
      );
      return args;
