<template>
  <v-app id="lex-web" v-bind:ui-minimized="isUiMinimized">
    ...
    <toolbar-container
      ...
      v-bind:is-ui-minimized="isUiMinimized"
      v-bind:is-ui-expanded="isUiExpanded"
      v-bind:userName="userNameValue"
      v-on:toggleIsUiExpanded="toggleIsUiExpanded"
      v-on:toggleMinimizeUi="toggleMinimizeUi"
...
export default {
  ...
  computed: {
    ...
    isUiExpanded() {
      return this.$store.state.isUiExpanded;
    },
    hasButtons() {
  ...
  created() {
    ...
    this.initConfig()
      .then(() => {
        // init vue store wil messages and lex client
        const promises = [
      ...
      .then(() =>
        //send 'ready' to parent window if running embedded
      ...
      .then(() => {
        // lex web ui is initalized, send utterance if !Iframe
        console.info(
  ...
  methods: {
    ...
    toggleIsUiExpanded() {
      return this.$store.dispatch("toggleIsUiExpanded");
    },
    ...
    messageHandler(evt) {
      ...
      switch (evt.data.event) {
        ...
        case "toggleMinimizeUi":
          ...
          break;
        case "toggleIsUiExpanded":
          this.$store.dispatch("toggleIsUiExpanded").then(() =>
            evt.ports[0].postMessage({
              event: "resolve",
              type: evt.data.event,
            })
        ...
        case "setSessionAttribute":
          /* eslint-disable */ console.log(
            ...oo_oo(
              `3280409281_439_10_439_72_4`,
              `From LexWeb: ${JSON.stringify(evt.data, null, 2)}`
            )
          );
          this.$store
...
/* sm-56px, md-48px, lg-64px 
Additional pixels added for footer */
